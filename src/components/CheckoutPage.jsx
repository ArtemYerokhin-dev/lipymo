...
function Field({ label, error, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {children}
      {error && <span className="text-[12px] text-brick">Обов'язкове поле</span>}
    </div>
  )
}
    ...
    <div className="min-h-screen bg-cream pt-[68px] flex items-center justify-center">
      <div className="text-center max-w-[480px] px-8">
        ...
        <h2 className="font-serif text-[36px] font-normal text-brown leading-tight mb-4">
          Замовлення<br /><em className="text-brick" style={{ fontStyle: 'italic' }}>прийнято!</em>
        </h2>
        <p className="text-[15px] text-brown-m font-light leading-[1.9] mb-8">
          Зв'яжемося з вами протягом 10 хвилин. Доставка — 2–4 години.
        </p>
        <button onClick={onHome} className="btn-primary">На головну</button>
      </div>
    </div>
  )
}
...
const PAYMENT_OPTIONS = [
  {
    id: 'cash',
    label: 'Готівка',
    sub: "Оплата кур'єру при отриманні",
    icon: (
...
export default function CheckoutPage({ items, onBack, onAdd, onRemove, onHome }) {
  const [form, setForm]       = useState({ name: '', phone: '', city: 'Київ', street: '', building: '', apt: '', note: '' })
  const [payment, setPayment] = useState('cash')
  const [card, setCard]       = useState({ number: '', holder: '', expiry: '', cvv: '' })
  const [errors, setErrors]   = useState({})
  const [done, setDone]       = useState(false)
  const [promoInput, setPromoInput] = useState('')
  const [promo, setPromo]           = useState(null)  // { code, discount } or null
  const [promoError, setPromoError] = useState(false)

  const PROMO_CODES = { 'ЛІПИМО': 20 }
  ...
  if (done)           return <SuccessScreen onHome={onHome} />
  if (!items.length)  return (
    <div className="min-h-screen bg-cream pt-[68px] flex items-center justify-center">
      <div className="text-center">
        <p className="text-[16px] text-brown-m font-light mb-6">Кошик порожній — спочатку оберіть страви.</p>
        <button onClick={onBack} className="btn-primary">До меню</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-cream pt-[68px]">
      ...
      <div className="bg-paper border-b border-oat px-5 md:px-[4.5rem] py-8">
        ...
        <div className="eyebrow">Майже готово</div>
        <h1 className="font-serif font-normal text-brown leading-[1.1]" style={{ fontSize: 'clamp(28px, 3vw, 44px)' }}>Куди везти?</h1>
      </div>
          ...
          <div className="info-card p-5 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label="Телефон *" error={errors.phone}>
                <Input value={form.phone} onChange={setF('phone')} placeholder="+380 XX XXX XX XX" type="tel" inputMode="tel" error={errors.phone} />
              </Field>
              <Field label="Місто">
                <Input value={form.city} onChange={setF('city')} />
              </Field>
              <Field label="Вулиця *" error={errors.street}>
                <Input value={form.street} onChange={setF('street')} placeholder="вул. Хрещатик" error={errors.street} />
              </Field>
              <Field label="Будинок *" error={errors.building}>
                <Input value={form.building} onChange={setFDigits('building')} placeholder="12" inputMode="numeric" error={errors.building} />
              </Field>
          ...
          <div className="info-card p-5 md:p-8">
            ...
            {payment === 'apple' && (
              <div className="border border-oat rounded-xl p-6 bg-cream flex gap-4">
                ...
                <div>
                  <div className="text-[14px] font-medium text-brown mb-1">Apple Pay потребує підключення</div>
                  <p className="text-[13px] text-brown-l font-light leading-[1.7]">
                    Для роботи потрібна інтеграція зі Stripe або LiqPay, HTTPS та верифікація домену у Apple.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="border border-oat rounded-xl p-6 bg-cream">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] text-brown-l font-light">Захищено SSL</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="info-card overflow-hidden">
              ...
              {items.map(({ product, qty }) => (
                <li key={product.id} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[14px] font-medium text-brown leading-tight">{product.name}</div>
                    <div className="text-[12px] text-brown-l mt-0.5">{product.weight}</div>
                  </div>
            ...
            <div className="px-5 md:px-7 pb-5 md:pb-7 pt-4 border-t border-oat">
              ...
              <div className="mb-5">
                {promo ? (
                  <div className="flex items-center justify-between bg-cream rounded-xl px-4 py-3">
                    <div>
                      <div className="text-[12px] tracking-[1.5px] uppercase text-brown-l mb-0.5">Промокод</div>
                ...
                {promoError && (
                  <div className="text-[12px] text-brick mt-1.5">Такого промокоду не існує</div>
                )}
              </div>

              <div className="flex justify-between mb-2">
                <span className="text-[13px] text-brown-l">Товари</span>
                <span className="text-[15px] text-brown">{subtotal} ₴</span>
              </div>
              {discount > 0 && (
                <div className="flex justify-between mb-2">
                  <span className="text-[13px] text-brick">Знижка {promo.discount}%</span>
                  <span className="text-[13px] text-brick">−{discount} ₴</span>
                </div>
              )}
              <div className="flex justify-between mb-6">
                <span className="text-[13px] text-brown-l">Доставка</span>
                <span className="text-[13px] text-brown-l">безкоштовно</span>
              </div>
              <div className="flex justify-between items-baseline mb-6 pb-6 border-b border-oat">
                <span className="text-[13px] tracking-[2px] uppercase text-brown-l">Разом</span>
              ...
              <button onClick={handleSubmit} className="w-full btn-primary py-4 text-center text-[14px]">
                {payment === 'cash' ? 'Підтвердити замовлення' : payment === 'apple' ? 'Оформити через Apple Pay' : `Оплатити ${total} ₴`}
              </button>
              <p className="text-[12px] text-brown-l text-center mt-4 font-light leading-[1.6]">
                Натискаючи кнопку, ви погоджуєтесь з умовами доставки
              </p>
            </div>
          </div>
        </div>
